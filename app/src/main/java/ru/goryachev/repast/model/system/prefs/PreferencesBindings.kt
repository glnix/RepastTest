package ru.goryachev.repast.model.system.prefs
...
private fun onGetPropertyFromClass(clazz: Class<*>): Preference<Any> {
    return when (clazz) {
        ...
        else -> throw UnsupportedOperationException("Unsupported preference typeName \"${clazz.canonicalName}\"")
    } as Preference<Any>
}
...
private fun onGetPreferencesFromSource(source: Any): SharedPreferences {
    return when (source) {
        ...
        else -> throw IllegalArgumentException("Unable to find \"SharedPreferences\" instance on typeName \"${source.javaClass.simpleName}\"")
    }
}
