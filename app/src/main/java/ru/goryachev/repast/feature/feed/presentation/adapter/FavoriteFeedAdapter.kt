...
class FavoriteFeedAdapter(itemsList: List<Any>,
    ...
    init {
        ...
        addXmlLayouter(R.layout.list_item_qr_sacanning) {
            matcher { it is ListItemQR }
            creator { view, viewGroup -> ListItemQRViewHolder(view, viewGroup) }
        }
    ...
    private inner class ListItemQRViewHolder(itemView: View, root: ViewGroup) : FieldViewHolder<ListItemQR>(itemView, root) {

        @SuppressLint("SetTextI18n")
        override fun performBind(data: ListItemQR) {
            itemView.findViewById<TextView>(R.id.scanBtn).setOnClickListener { data.clickListener.invoke() }
        }
    }
}