package ru.goryachev.repast.feature.global.presentation.view.adapter
...
abstract class LoadingAdapter<T : Any> : FieldsAdapter<Any>() {
    ...
    companion object {
        ...
        private val SPAN_FULL_ITEM = Any()
    }
    ...
    override fun onViewAttachedToWindow(holder: FieldViewHolder<Any>) {
        super.onViewAttachedToWindow(holder)
        val itemView = holder.itemView
        val tag = itemView.tag
        val lp = itemView.layoutParams
        if (tag == SPAN_FULL_ITEM && lp is StaggeredGridLayoutManager.LayoutParams) {
            lp.isFullSpan = true
        }
    }
